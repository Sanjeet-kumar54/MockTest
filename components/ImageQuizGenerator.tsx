...
import { Camera, Sparkles, FileText } from './icons/Icons';
import { analyzeImageForQuiz } from '../services/geminiService';

interface ImageQuizGeneratorProps {
  onTestGenerated: (title: string, questions: Question[], config?: { posMarks?: number, negMarks?: number, duration?: number }) => void;
}

export const ImageQuizGenerator: React.FC<ImageQuizGeneratorProps> = ({ onTestGenerated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const questions = await analyzeImageForQuiz(file, answerSheet || undefined);
        if (questions && questions.length > 0) {
            onTestGenerated(
                `Mock Test from ${file.name}`, 
                questions,
                {
            ...
            setError("Could not find any questions in the image. Please try another one.");
        }
    } catch (err) {
    ...
    <div className="max-w-3xl mx-auto">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl transition-colors duration-200">
        <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold mb-2 text-gray-900 dark:text-white">Image to Mock Test Generator</h1>
            <p className="text-gray-600 dark:text-gray-300">
            Upload an image of a question from a book or your notes. Gemini will create a quick mock test for you.
            </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
           {/* Main Image Upload */}
          <label htmlFor="image-upload" className="cursor-pointer block w-full p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-center transition-colors">
            {preview ? (
              <img src={preview} alt="Preview" className="max-h-48 mx-auto rounded-md" />
            ) : (
                <span className="text-gray-500 dark:text-gray-400">Click to upload Question Image</span>
            )}
           ...
           <div className="bg-gray-50 dark:bg-gray-700/30 p-6 rounded-xl border border-gray-100 dark:border-gray-700 space-y-4">
             ...
             <div>
                 ...
                 <label htmlFor="answer-sheet-upload-img" className="cursor-pointer flex items-center justify-center w-full p-3 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg hover:bg-white dark:hover:bg-gray-600 text-center transition-colors bg-white dark:bg-gray-700">
                    {answerSheet ? (
                    <span className="text-secondary text-sm font-semibold flex items-center gap-2">
                        <FileText className="w-4 h-4"/> {answerSheet.name}
                    </span>
                    ) : (
                    <span className="text-gray-400 text-sm">Click to upload Answer Sheet (Image)</span>
                    )}
                    <input id="answer-sheet-upload-img" type="file" accept="image/*" className="hidden" onChange={handleAnswerSheetChange} />
                </label>
             </div>
          </div>
          ...
          <button 
            ...
            className="w-full bg-secondary text-white font-bold py-3 px-6 rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-wait flex items-center justify-center shadow-lg"
          >
            {isGenerating ? (
              <>
